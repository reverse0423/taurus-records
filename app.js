...
function bindEvents() {
  ...
  ["gameDate", "opponent", "stadium", "ourScore", "opponentScore"].forEach(id => document.getElementById(id).addEventListener("input", syncGameMeta));
...
function renderGameState() {
  ...
  byId("gameStateText").textContent = `${inning}회${half === "top" ? "초" : "말"} · ${half === "top" ? "우리 공격" : "상대 공격"} · ${outs}아웃`;
}
...
function renderOurBatters() {
  renderEditableTable("ourBattersTable", batterFields, state.currentGame.batters, "batter", state.currentGame.live.ourBatterIndex);
}

function renderOpponentBatters() {
  renderEditableTable("opponentBattersTable", opponentFields, state.currentGame.opponentBatters, "opponentBatter", state.currentGame.live.opponentBatterIndex);
}
...
function addOutsAndMaybeSwitch(side, outs) {
  ...
  if (side === "our") {
    ...
    showTab("opponent");
    showToast("3아웃입니다. 상대 공격으로 전환했습니다.");
  } else {
    state.currentGame.live.half = "top";
    state.currentGame.live.inning += 1;
    showTab("our");
    showToast("3아웃입니다. 우리 공격으로 전환했습니다.");
  }
}
...
function getCurrentBatter(side) {
  ...
  const key = side === "our" ? "ourBatterIndex" : "opponentBatterIndex";
...
function advanceBatter(side) {
  const rows = side === "our" ? state.currentGame.batters : state.currentGame.opponentBatters;
  const key = side === "our" ? "ourBatterIndex" : "opponentBatterIndex";
  if (rows.length) state.currentGame.live[key] = (state.currentGame.live[key] + 1) % rows.length;
}
...
function validateGame() {
  ...
  if (awayRuns !== toInt(state.currentGame.score.ours)) issues.push(`우리 점수(${state.currentGame.score.ours})와 이닝 합계(${awayRuns})가 다릅니다.`);
  if (homeRuns !== toInt(state.currentGame.score.opponent)) issues.push(`상대 점수(${state.currentGame.score.opponent})와 이닝 합계(${homeRuns})가 다릅니다.`);
...
function exportDetailJson() {
  ...
  const detail = {
    [detailId]: {
      ...
      status: status || "",
      teams: {
        away: { name: TEAM_NAME, abbr: "CBNU" },
        home: { name: opponentName, abbr: makeAbbr(opponentName) }
      },
      innings: normalizeDetailInnings(game),
      batting: {
        away: game.batters.map(toDetailBatter),
        home: game.opponentBatters.map(toDetailBatter)
      },
      pitching: {
        away: game.pitchers.map(toDetailPitcher),
        home: game.opponentPitchers.map(toDetailPitcher)
      },
      plays: game.plays || []
    }
  };
